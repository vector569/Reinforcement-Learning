...
def action(state,Q,Q2,u,n_episodes,epsilon=0.2):
	temp = []
	for i in range(len(Q)):
		temp.append([Q[i][0],Q[i][1],Q[i][2]+Q2[i][2]])
...
for u in range(n_episodes):
	s = obj.start_state
	a = action(s,Q,Q2,u,n_episodes,epsilon)
	trail = [s]
	reward = 0
	while(s != obj.terminal_state):
		[s_p,R] = obj.take_action(s,a)
		reward += R
		a_p = action(s_p,Q,Q2,u,n_episodes,epsilon)

		temp1 = np.random.choice(2, 1, p=[0.5,0.5])
		if temp1 == 1:
			max_ = -99999
			index = 0
			for i in range(len(Q)):
			...
			for i in range(len(Q)):
				if Q[i][0] == s and Q[i][1] == a:
					Q[i][2] = Q[i][2] + alpha*(1-u/n_episodes)*(R + gamma*Q2[index][2] - Q[i][2])

		else:
			max_ = -99999
			index = 0
			for i in range(len(Q)):
				if Q2[i][0] == s_p and Q2[i][2]>max_:
					max_ = Q2[i][2]
					index = i

			for i in range(len(Q)):
				if Q2[i][0] == s and Q2[i][1] == a:
					Q2[i][2] = Q2[i][2] + alpha*(1-u/n_episodes)*(R + gamma*Q[index][2] - Q2[i][2])
